...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if (conf["isim-yas"]) {
    ...
    if (!eski) {
    message.channel.send(embed.setDescription(`<@${uye.user.id}> başarıyla <@&787331149994393651> olarak kayıt edildi`)).then(x => x.delete({timeout: 15000}));
  }
  if (eski) {
    message.channel.send(embed.setDescription(`<@${uye.user.id}> başarıyla <@&787331149994393651> olarak kayıt edildi \n 
